...
echo " DATEBASE  Connected ";
 ...
 $currentDate = date('m/d/Y');
  ...
  //$timestamp = time();
  //$currentDate = gmdate('m/d/y', $timestamp);
  echo $currentDate;
...
if ($datebins > $datefins){
    ...
    if($birth>$currentDate){
        echo '<script type ="text/JavaScript">';  
        echo 'alert("the date of birth is wrong")'; 
        echo '</script>'; 
    }else{
        ...
        $sql= "INSERT INTO client (id, fullname, passw, firstname, familyname, birth, eemail, ephone, ecity, ereg, estr, codep, ssn, icn)".
              " VALUES ('$id', '$fullname', '$passw', '$firstname', '$familyname', '$birth', '$eemail', '$ephone', '$ecity', '$ereg', '$estr', '$codep', '$ssn', '$icn')";
        ...
        $sqll= "INSERT INTO car (idcar,id, carnum,carname,carcoler,mileage,insn,inst,datebins,datefins)".
              " VALUES ('$idcar','$id', '$carnum','$carname', '$carcoler', '$mileage', '$insn', '$inst', '$datebins', '$datefins')";
        $res=$conn->query($sqll);
        if(!$res){
            $errorMessage="Invalid query: " . $conn->error;
            break;
...
<body>
    ...
    <form method="post">
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-6">
                <input type="password" class="form-control" name="passw"  placeholder=".......... (just 10 values )" pattern="[a-z0-9._%+\-]{10}" value="<?php echo $passw; ?>">
            </div>
        </div>
